...
import { donationService, userService, type DonationInfoDto } from '../../api';
import DonationFilter from '../../components/DonationFilter';
import Modal from '../../components/UI/Modal';
import EmptyBasketSVG from '../../components/UI/EmptyBasketSVG';

const PLACEHOLDER_IMAGE = 'https://images.unsplash.com/photo-1560806887-1e4cd0b6cbd6?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80';
...
const AdminDonationCard = ({
  ...
  const expiration = donation.expirationDate ? new Date(donation.expirationDate).getTime() : Infinity;
  const daysLeft = Math.ceil((expiration - Date.now()) / 86400000);
  const isExpiringSoon = isFinite(expiration) && daysLeft <= 1 && daysLeft >= 0;
  ...
  return (
    <div
      ...
      <div className="relative h-[200px] overflow-hidden bg-gray-100 dark:bg-gray-800 flex-shrink-0">
        <img
          src={donation.image || PLACEHOLDER_IMAGE}
          alt={donation.title}
        ...
        <span className={`absolute top-3 right-3 px-2.5 py-1 rounded-lg text-[11px] font-bold shadow-sm ${
          isAvailable ? 'bg-green-500/80 text-white' : 'bg-amber-500/80 text-white'
        }`}>
          {donation.status}
        </span>
      </div>
      ...
      <div className="flex flex-col flex-grow p-4 pt-3.5">
        <h3 className={`text-lg font-bold leading-snug mb-1.5 ${theme === 'light' ? 'text-[#1a1a1a]' : 'text-white'}`}>
          {donation.title}
        </h3>
        <p className={`text-[13px] mb-3 line-clamp-2 leading-relaxed ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className={`text-xs font-medium truncate ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>
            by {donorName}
          </p>
        </div>

        <div className={`mt-auto space-y-2 pt-3 border-t ${theme === 'light' ? 'border-gray-100' : 'border-[#2e2e2e]'}`}>
          <div className={`flex items-center text-[13px] ${theme === 'light' ? 'text-gray-600' : 'text-gray-300'}`}>
...
const AdminDonationFeed: React.FC = () => {
  ...
  const [donations, setDonations] = useState<DonationInfoDto[]>([]);
  ...
  const filteredDonations = useMemo(() => {
    return donations.filter(d => {
      const donorName = userMap[d.donorId] || '';
      const matchSearch = d.title.toLowerCase().includes(searchQuery.toLowerCase()) || donorName.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const confirmDelete = async () => {
    ...
    try {
      await donationService.delete(donationToDelete.id);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto min-h-screen relative animate-fade-in-up">
      ...
      {isFilterOpen && (
        <div
          className={`fixed inset-0 z-10 ${theme === 'light' ? 'bg-gray-900/10' : 'bg-black/30'}`}
      ...
      {loading ? (
        ...
        <div className={`flex flex-col items-center justify-center min-h-[50vh] rounded-2xl border border-dashed relative z-0 ${
          ...
          <div className={`p-6 rounded-2xl mb-4 ${theme === 'light' ? 'bg-violet-50' : 'bg-violet-900/10'}`}>
            <EmptyBasketSVG className="w-20 h-20 text-[#8b5cf6]" />
          </div>
          <p className={`font-semibold text-lg ${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>
      ...
      <Modal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} title="Confirm Deletion">
        <div className="text-center">
          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/20 mb-4">
            <TrashIcon className="h-8 w-8 text-red-600 dark:text-red-500" />
          </div>
          <p className={`text-sm mb-6 mt-2 ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>
            Are you sure you want to permanently delete <strong>{donationToDelete?.title}</strong>? This action cannot be undone.
          </p>
