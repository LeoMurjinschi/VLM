...
import { CheckCircleIcon, InformationCircleIcon, StarIcon, ExclamationCircleIcon } from '@heroicons/react/24/solid';
import { dashboardApiService } from '../api/dashboardApiService';
import type { ActivityItemDto } from '../api/dashboardApiService';
...
const RecentActivity: React.FC = () => {
  ...
  useEffect(() => {
    ...
    dashboardApiService.getActivity(donorId, 5)
      .then(setActivities)
      .catch(() => setActivities([]))
  ...
  return (
    <div className={`p-6 md:p-8 rounded-3xl border shadow-[0_8px_30px_rgb(0,0,0,0.04)] animate-fade-in-up delay-200 ${
      ...
      {loading ? (
        ...
        <p className={`text-sm text-center py-6 ${theme === 'light' ? 'text-gray-400' : 'text-gray-500'}`}>
          No activity yet. Start by adding donations!
        </p>
      ) : (
