...
const DonationFeed: React.FC = () => {
  ...
  let processedDonations = [...MOCK_DONATIONS];
          ...
          <button 
            ...
            className={`px-3 sm:px-4 py-3.5 border rounded-xl shadow-sm transition-all flex items-center gap-2 font-bold flex-shrink-0
              ${isFilterOpen || selectedCategories.length > 0 || statusFilter !== 'All' 
                ? 'bg-blue-600 border-transparent text-white shadow-blue-200' 
      ...
      {processedDonations.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center min-h-[50vh] bg-white rounded-3xl border border-dashed border-gray-300 relative z-0">
          ...
          <button 
            ...
            className="mt-6 px-6 py-2 bg-blue-100 text-blue-700 font-bold rounded-lg hover:bg-blue-200 transition-colors"
          >
            Clear all filters
          </button>
        </div>
