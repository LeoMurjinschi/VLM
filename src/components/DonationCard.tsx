...
interface DonationCardProps {
  donation: Donation;
  onReserve: (id: string, amount: number) => void; // <-- PRIMIM FUNCȚIA AICI
}
...
const DonationCard: React.FC<DonationCardProps> = ({ donation, onReserve }) => {
  ...
  return (
    <>
      <div className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 flex flex-col h-full relative z-0">
        ...
        <div className="p-6 flex flex-col flex-grow">
          ...
          <h3 className="text-xl font-extrabold text-gray-900 mb-2 leading-tight group-hover:text-blue-600 transition-colors">{donation.title}</h3>
          <p className="text-sm text-gray-500 mb-5 line-clamp-2 leading-relaxed">{donation.description}</p>
          ...
          <button 
            ...
            disabled={isReserved}
            className={`w-full mt-6 py-3 px-4 rounded-xl font-bold transition-all active:scale-[0.98] ${isReserved ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-200 hover:shadow-blue-300'}`}
          >
